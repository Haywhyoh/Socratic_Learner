...
class LangChainCoachLLM:
    ...
    def evaluate_answer(
        ...
        *,
        ...
        prompt = (
            "You evaluate a learner's answer to ONE diagnostic question. "
            "Return ONLY JSON: {\"passed\": true|false, \"push_back\": null|\"one sentence\"}. "
            "passed=true if they correctly answer THIS question, even if the answer is short "
            "(for example 'None' when asked what the caller receives with no return). "
            "Do not require a full concept essay. If they are wrong, passed=false and "
            "push_back is one short sentence that names the mistake without lecturing. "
            "Never give code. Never ask a different question.\n"
            f"Milestone: {milestone_title}\n"
            f"Question: {question}\n"
            f"Answer: {answer}\n"
        )
    ...
    def evaluate_explanation(
        ...
        *,
        ...
        prompt = (
            ...
            "concept. A short correct answer to that question is a pass. Stay inside this "
            "concept — do not fail them for omitting a later idea. "
            ...
            "is wrong, passed=false. Do not praise the wrong part. If passed=false, "
            "feedback must tell them to try the current question again.\n"
    ...
    def generate_concept_content(
        ...
        *,
        ...
        prompt = (
            ...
            "practice_tasks must use id, filename (practice/<id>.<ext>), prompt, run, expect, "
            "rubric — not title/description/acceptance_criteria. "
...
def _dynamic_mentor_prompt(context: dict[str, Any], message: str, action_hint: str) -> str:
    ...
    return (
        ...
        f"{language_notes}"
        f"{task_block}"
        "You may call list_workspace_files, read_workspace_file, "
        "delete_workspace_file, rename_workspace_file, or run_workspace_command "
        "if you need to inspect, rename, delete, or run their code before writing the JSON. "
        "File tools only work inside the learner sandbox; never use paths with ..\n"
